...
RxIoSampleBaseResponse::RxIoSampleBaseResponse() :
        RxResponse() {
...
bool RxIoSampleBaseResponse::containsDigital() {
    return (getFrameData()[getSampleOffset() + 1] & 0x1) > 0
            || getFrameData()[getSampleOffset() + 2] > 0;
}
...
Rx16IoSampleResponse::Rx16IoSampleResponse() :
...
Rx64IoSampleResponse::Rx64IoSampleResponse() :
        RxIoSampleBaseResponse() {
    _remoteAddress = XBeeAddress64();
}
...
void XBeeResponse::getRx64IoSampleResponse(XBeeResponse &response) {
    ...
    rx->getRemoteAddress64().setMsb(
            (uint32_t(getFrameData()[0]) << 24)
                    + (uint32_t(getFrameData()[1]) << 16)
                    + (uint16_t(getFrameData()[2]) << 8) + getFrameData()[3]);
    rx->getRemoteAddress64().setLsb(
            (uint32_t(getFrameData()[4]) << 24)
                    + (uint32_t(getFrameData()[5]) << 16)
                    + (uint16_t(getFrameData()[6]) << 8) + getFrameData()[7]);
}
...
bool XBee::available() {
    return _serial->IOKbHit();
}

uint8_t XBee::read() {
    uint8_t b;
    _serial->Read(&b, 1, 2000);
    return b;
...
bool XBee::readPacket(int timeout) {
    ...
    while (int(diffclock(start, clock())) < timeout) {
...
PayloadRequest::PayloadRequest(uint8_t apiId, uint8_t frameId, uint8_t *payload,
...
ZBTxRequest::ZBTxRequest(XBeeAddress64 &addr64, uint16_t addr16,
        uint8_t broadcastRadius, uint8_t option, uint8_t *data,
...
ZBTxRequest::ZBTxRequest(XBeeAddress64 &addr64, uint8_t *data,
        uint8_t dataLength) :
...
Tx16Request::Tx16Request() :
        PayloadRequest(TX_16_REQUEST, DEFAULT_FRAME_ID, NULL, 0) {

}

Tx16Request::Tx16Request(uint16_t addr16, uint8_t option, uint8_t *data,
        uint8_t dataLength, uint8_t frameId) :
...
Tx16Request::Tx16Request(uint16_t addr16, uint8_t *data, uint8_t dataLength) :
        PayloadRequest(TX_16_REQUEST, DEFAULT_FRAME_ID, data, dataLength) {
    _addr16 = addr16;
    _option = ACK_OPTION;
}
...
Tx64Request::Tx64Request(XBeeAddress64 &addr64, uint8_t option, uint8_t *data,
        uint8_t dataLength, uint8_t frameId) :
...
Tx64Request::Tx64Request(XBeeAddress64 &addr64, uint8_t *data,
        uint8_t dataLength) :
...
AtCommandRequest::AtCommandRequest(uint8_t *command, uint8_t *commandValue,
